                ...
                <div class="row">
                    <div class="col-md-3" style="padding:0px;">
                        ...
                        @foreach($kategori as $kategori)
                            <p><a class="text-muted" href="/dosen/daftarbacaan/jenis/{{ $kategori -> id_kategori }}" style="text-decoration: none;">{{ $kategori -> nama_kategori }}</a></p>
                        @endforeach
                        <br><br>
                        <p style="font-weight: 700; font-size:25px; color: #8cbee8">Topik</p><hr>
                        @foreach($topik as $topik)
                            <p><a class="text-muted" href="/dosen/daftarbacaan/topik/{{ $topik -> id_topik }}" style="text-decoration: none;">{{ $topik -> nama_topik }}</a></p>
                        @endforeach
                    </div>
                    <div class="col-md-9">
                        <center><h1 style="font-weight: 700; color: #413A3A">Hasil Pencarian Bacaan</h1></center><br><hr><br>
                        @foreach($bacaan as $bacaan)
                            @if($bacaan-> judul <> "")
                            <p style="text-align:justify;"><a href="/dosen/detail/{{ $bacaan-> id_bacaan }}">{{ $bacaan-> judul }}</a></p>
                            <p style="text-align:justify; font-size:14px">Penulis : {{ $bacaan-> penulis }}</p>
                            <p><a class="text-muted" href="/tampilpdf/{{ $bacaan-> file }}" style="text-decoration: none;"  target="_blank">[ File ]</a></p>
                            @elseif($bacaan-> judul <> "null")
                            @endif
                        @endforeach
                    </div>
                </div>
